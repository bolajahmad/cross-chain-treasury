...
import {ActionCreated, TreasuryExecution} from './model'
...
const processor = new EvmBatchProcessor()
  ...
  .addLog({
    ...
    topic0: [treasuryAbi.events.ActionCreated.topic, treasuryAbi.events.TreasuryExecution.topic],
  })
...
processor.run(db, async (ctx) => {
  ...
  for (let block of ctx.blocks) {
    ...
    for (let log of block.logs) {
      if (log.address === TREASURY_CONTRACT_ADDRESS) {
        // SQD's very own EVM codec at work - about 20 times faster than ethers
        console.log({ log });
        // let {from, to, value} = treasuryAbi.events.ActionCreated.decode(log)
        // actions.push(new ActionCreated({
        //   id: log.id,
